...
class dualVAE(keras.Model):
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape() as tape:
            ...
            convergence_loss = keras.losses.mse(zy_mean, zx)
...
def createDualVAE(params=None):
    ...
    encoderInput = keras.Input(shape=params['INPUT_SHAPE'], name='encoder_input')
    x = keras.layers.Conv1D(filters=params['FILTER_SIZES'][0], kernel_size=params['KERNEL_SIZES'][0], strides=2, padding='same', name='Conv1D_0')(encoderInput)
    x = keras.layers.LeakyReLU(alpha=params['ALPHA'], name='LeakyReLU_0')(x)
    x = keras.layers.Dropout(params['DROPOUT'], name='Dropout_0')(x)
    x = keras.layers.Conv1D(filters=params['FILTER_SIZES'][1], kernel_size=params['KERNEL_SIZES'][1], strides=2, padding='same', name='Conv1D_1')(x)
    x = keras.layers.LeakyReLU(alpha=params['ALPHA'], name='LeakyReLU_1')(x)
    x = keras.layers.Dropout(params['DROPOUT'], name='Dropout_1')(x)
    x = keras.layers.Conv1D(filters=params['FILTER_SIZES'][2], kernel_size=params['KERNEL_SIZES'][2], strides=2, padding='same', name='Conv1D_2')(x)
    x = keras.layers.LeakyReLU(alpha=params['ALPHA'], name='LeakyReLU_2')(x)
    x = keras.layers.Dropout(params['DROPOUT'], name='Dropout_2')(x)
    x = keras.layers.Conv1D(filters=params['FILTER_SIZES'][3], kernel_size=params['KERNEL_SIZES'][3], strides=2, padding='same', name='Conv1D_3')(x)
    x = keras.layers.LeakyReLU(alpha=params['ALPHA'], name='LeakyReLU_3')(x)
    x = keras.layers.Flatten(name='Flatten_0')(x)
    z = keras.layers.Dense(params['LATENT_SIZE'], activation='linear', name='z')(x)

    vencoderInput = keras.Input(shape=params['INPUT_SHAPE'], name='v_encoder_input')
    x = keras.layers.Conv1D(filters=params['FILTER_SIZES'][0], kernel_size=params['KERNEL_SIZES'][0], strides=2, padding='same', name='Conv1D_0')(vencoderInput)
    x = keras.layers.LeakyReLU(alpha=params['ALPHA'], name='LeakyReLU_0')(x)
    x = keras.layers.Dropout(params['DROPOUT'], name='Dropout_0')(x)
    x = keras.layers.Conv1D(filters=params['FILTER_SIZES'][1], kernel_size=params['KERNEL_SIZES'][1], strides=2, padding='same', name='Conv1D_1')(x)
    x = keras.layers.LeakyReLU(alpha=params['ALPHA'], name='LeakyReLU_1')(x)
    ...
    decoderInput = keras.Input(shape=(params['LATENT_SIZE'],), name='decoder_input')
    x = keras.layers.Dense(15*params['FILTER_SIZES'][-1], activation='linear', name='Dense_0')(decoderInput)
    x = keras.layers.Reshape((15, params['FILTER_SIZES'][-1]))(x)
    x = keras.layers.Conv1DTranspose(filters=params['FILTER_SIZES'][2], kernel_size=params['KERNEL_SIZES'][3], activation='relu', strides=2, padding='same', name='Conv1D_0')(x)
    x = keras.layers.Conv1DTranspose(filters=params['FILTER_SIZES'][1], kernel_size=params['KERNEL_SIZES'][2], activation='relu', strides=2, padding='same', name='Conv1D_1')(x)
    x = keras.layers.Conv1DTranspose(filters=params['FILTER_SIZES'][0], kernel_size=params['KERNEL_SIZES'][1], activation='relu', strides=2, padding='same', name='Conv1D_2')(x)
    decoderOutput = keras.layers.Conv1DTranspose(filters=1, kernel_size=params['KERNEL_SIZES'][0], activation='linear', strides=2, padding='same', name='Conv1D_3')(x)
